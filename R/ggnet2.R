...
ggnet2_interactive <- function(
  ...
  require("network", character.only = T)
  require("sna", character.only = T)
  ...
  p = ggplot(data, aes(x = x, y = y, data_id = label))
  # p = ggplot(data, aes(x = x, y = y, tooltip = label, data_id = label))
  ...
  p = p +
    # geom_point(aes(alpha = factor(alpha), color = factor(color), shape = factor(shape), size = factor(size)))
    geom_point_interactive(aes(alpha = factor(alpha), color = factor(color), shape = factor(shape), size = factor(size)))
